...
def main():
    if len(sys.argv) < 4:
        print(
            f"Usage: {sys.argv[0]} <input.json> <output.json> <pattern1> [pattern2] ...",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    include_patterns = [
        p for p in all_patterns if not p.startswith("!") and not p.startswith("~")
    ]
    exclude_patterns = [
        p[1:] for p in all_patterns if p.startswith("!") and not p.startswith("~")
    ]
